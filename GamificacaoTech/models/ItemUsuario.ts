...
export = class ItemUsuario {
    ...
    public static async obterItensUsuario(ra: number): Promise<ItemUsuario[]> {
        ...
        await Sql.conectar(async (sql: Sql) => {
            lista = await sql.query("select i.id_item, i.nome_item, i.img_url_item from item_usuario u, item where fk_usuario_id = ? and u.fk_item_id = i.id_item", [ra]) as ItemUsuario[]
        })
        return lista
    }
