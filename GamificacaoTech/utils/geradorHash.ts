...
export = class GeradorHash {
    ...
    public static async criarHash(senha: string): Promise<string> {
        return new Promise<string>((resolve, reject) => {
            ...
            pbkdf2(Buffer.from(senha), salt, 4096, GeradorHash.HASH_BYTE_SIZE, "sha512", (err, derivedKey) => {
                if (err) {
                    reject(err)
                    return
                }
    ...
    public static async validarSenha(senha: string, hash: string): Promise<boolean> { //senha = input que o usuário colocou no form //hash = senha guardada no banco
        return new Promise<boolean>((resolve, reject) => {
            ...
            pbkdf2(Buffer.from(senha), Buffer.from(saltHash, "base64"), 4096, GeradorHash.HASH_BYTE_SIZE, "sha512", (err, derivedKey) => {
                if (err) {
                    reject(err)
                    return
                }
